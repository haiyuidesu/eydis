...
{
    char *prologues[] = {
        "\xf6\x57\xbd\xa9", "\xfd\x7b\xbf\xa9", "\xfc\x6f\xba\xa9",
...
unsigned int insn_is_unconditional(uint32_t mem)
{
    if ((mem & 0xfe1f0000) == 0xd61f0000) {
        ...
        if (rm(mem) == 0x1f && (imms(mem) & 0x3e) == 0x2) {
            ...
            if ((rn(mem) != 0x1f) && (op7(mem) == 0x2 || op7(mem) == 0x4)) {
                return 0;
            }
    ...
    if ((mem & 0x3b200c00) == 0x38200800) {
        ...
        if (rm(mem) != 0x1f) {
            ...
            if (opt(mem) == 0x3) {
                ...
                if (shift) {
                    print_unsigned_immediate(shift);
                }
            }
        }
...
unsigned int insn_is_float_point_int_conversion(uint32_t mem)
{
    if ((mem & 0x5f20fc00) == 0x1e200000) {
        ...
        if (((op0(mem) & 0x1) || (op0(mem) & 0x2)) && (((op1(mem) & 0x6) == 0x2) || ((op1(mem) & 0x6) == 0x4)))
            return 0;
